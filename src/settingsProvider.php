...
class SettingsProvider
{
    ...
    function __construct(Logger $logger, string $path = null)
    {
        ...
        if ($this->logger->getVerbosity() >= 1) {
            $log_message = sprintf("SettingsProvider: New SettingsProvider instance created. (Settings path: '%s')", $this->path);
     ...
     */
    function createSettings(AdminHandler $admin_handler = null, string $administration_password = "password", bool $check_ip = true, string $parse_mode = "HTML", bool $send_actions = true, bool $use_test_api = false, bool $maintenance_enabled = false, string $maintenance_message = "Bot in maintenance, please wait.", bool $antiflood_enabled = true, int $antiflood_seconds = 5, int $antiflood_messages_number = 5, int $antiflood_ban_seconds = 120, string $antiflood_ban_message = "Flood detected, you're banned for 2 minutes."): self
    {
        if (null == $admin_handler) $admin_handler = new AdminHandler([]);
        $hash_info = password_get_info($administration_password);
        if (1 != $hash_info['algo']) $administration_password = password_hash($administration_password, PASSWORD_DEFAULT);
        ...
        $antiflood = new Sections\AntiFloodSection($antiflood_enabled, $antiflood_seconds, $antiflood_messages_number, $antiflood_ban_seconds, $antiflood_ban_message);
    ...
    {
        if ($this->logger->getVerbosity() >= 1) $this->logger->log("SettingsProvider: Settings instance returned.");
    ...
    function loadSettings(): self
    {
        ...
        $general_section = new Sections\GeneralSection($admin_handler, $settings->general->administration_password, $settings->general->check_ip);
        ...
        if ($this->logger->getVerbosity() >= 1) {
            $log_message = sprintf("SettingsProvider: Settings loaded from file at path '%s'. (%s)", $this->path, $settings_file);
            $this->logger->log($log_message);
        }
        return $this;
    }
